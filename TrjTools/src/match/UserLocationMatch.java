...
public class UserLocationMatch {
	...
	public static final String NORTH2SOUTH ="自北向南"; 
	public static final String SOUTH2NORTH ="自南向北"; 
	public static final String EAST2WEST ="自东向西"; 
	public static final String WEST2EAST ="自西向东"; 
	...
	private static List<UserLocation> users = new ArrayList<UserLocation>();
	...
	public static void main(String[] args) {
		...
		for(UserLocation user:users){
			...
			result.add(roads);
			System.out.println(user.getLat()+","+user.getLng() +"\t"+ user.getDirection());
...
//		GeoPoint a = new GeoPoint(39.908051,116.4599);
//		GeoPoint b = new GeoPoint(39.908674,116.459644);
//		String d = UserLocationMatch.EAST2WEST;
//		determineDirection(a, b, d);
	}
	...
	private static List<MatchResult> userMatch(UserLocation user){
...
//			System.out.println(user.getLng()+","+user.getLat());
	...
	private static HashSet<MatchResult> getCandidateEdges(UserLocation user, double radius)
    {
        double maxRadius = MAX_RADIUS;
        HashSet<Edge> cands = graph.rangeQuery(user.getLocation(), radius, maxRadius);
        HashSet<MatchResult> result = new HashSet<MatchResult>();
        for(Edge e : cands)
        {
        	DistanceType dt = e.projectFrom(user.getLocation());
            if (dt.type == 0)
            {
            	GeoPoint start;
            	GeoPoint end;
...
//            	}
            	if(determineDirection(start,end,user.getDirection())){
//            	if(determineDirection(start, end, user.getLocation())){
	...
	private static boolean determineDirection(GeoPoint start,GeoPoint end,String direction){
		//构造一个纬度与起点相同，经度与终点相同的点，以方便计算起终点之间的经度差距离和纬度差距离
		GeoPoint temp = new GeoPoint(start.getLat(), end.getLng());
		
		double latDelta = GeoPoint.getPreciseDistance(end, temp);
		double lngDelta = GeoPoint.getPreciseDistance(start, temp);
		
		boolean northsouth = ((latDelta-lngDelta)>0);
		
		if(northsouth){
			if(start.getLat()>end.getLat()&&direction.equals(UserLocationMatch.NORTH2SOUTH)){
				return true;
			}else if(start.getLat()<end.getLat()&&direction.equals(UserLocationMatch.SOUTH2NORTH)){
				return true;
			}else{
				return false;
			}
		}else{
			if(start.getLng()>end.getLng()&&direction.equals(UserLocationMatch.EAST2WEST)){
				return true;
			}else if(start.getLng()<end.getLng()&&direction.equals(UserLocationMatch.WEST2EAST)){
				return true;
			}else{
				return false;
			}
...
//			BufferedReader reader = new BufferedReader(new InputStreamReader(UserLocationMatch.class.getClass().getResourceAsStream(getTestSourceFile())));
			...
			while((line=reader.readLine())!=null){
				...
				users.add(new UserLocation(new GeoPoint(lat, lng), line.split("\t")[1]));
			}
			reader.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();	
